...
class mainMenu(scene):
    ...
    def mount(self):
        ...
        # opcoes              
        mainOptions = options(20, 300, engine.WHITE, engine.ORANGE, 'Game/pyGameEngine/assets/fonts/MountainsofChristmas-Regular.ttf') 
        mainOptions.addItem("Start", "stage1")
        mainOptions.addItem("Options", "optionsmenu")
        mainOptions.selected(0)
        self.addEntity(mainOptions)
        # controles
        self.input.keyboard.addHandler(self.input.keyboard.UP, mainOptions, 'up')
        self.input.keyboard.addHandler(self.input.keyboard.DOWN, mainOptions, 'down')
        self.input.keyboard.addHandler(self.input.keyboard.RETURN, mainOptions, 'do', self.execute)
        self.input.register(self.input.keyboard.getHandler())
        # sons 
        self.media.addSound('Opening', 'Game/pyGameEngine/assets/sounds/opening.mp3', -1, 0.2)
        self.media.addSound('Doors', 'Game/pyGameEngine/assets/sounds/doors.mp3', -1, 0.5)
